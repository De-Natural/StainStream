...
export default function Moiveline( { title = 'Series', style, customStyle = {} } ) {
    const Arritems = [
        {
            img: logo3,
    ...
    const { wrapper: wrapperStyle } = customStyle;
    return (
        <div className='h-[350px] w-full flex items-center'  style={wrapperStyle}>
            <p className='text-3xl text-white font-sans translate-y-[-150px] mt-6 translate-x-20' style={style} >{title}</p>
            <Swiper
                modules={[Navigation]}
                navigation={true}
                    ...
                    <SwiperSlide key={index}>
                        <section className='h-full w-56 mt-8 flex flex-col items-center justify-around'>
                            <figure className='h-48 w-full'>
                                <img className='h-full w-full object-cover' src={item.img} alt={item.pt} />
                            </figure>
